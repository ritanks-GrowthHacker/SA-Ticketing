...
export default function OrganizationVerifyPage() {
  ...
  const handleVerifyOTP = async (otpCode: string) => {
    ...
    try {
      const response = await fetch('/api/org-verify-email', {
        method: 'POST',
        headers: {
      ...
      if (response.ok && data.success) {
        ...
        setTimeout(() => {
          router.push('/org-login?verified=true');
        }, 2000);
      } else {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      // Call the org-onboarding API with just the email to resend OTP
      const response = await fetch('/api/org-resend-otp', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ orgEmail: email }),
      });
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="text-center">
            ...
            <p className="mt-2 text-center text-sm text-gray-600">
              Your organization email has been verified. Redirecting to login...
            </p>
          </div>
        </div>
      </div>
